...
public class HomeController {
	...
	public String mostrarIndex(Authentication auth, HttpSession session) {
		...
		// con este for mostramos el nombre de roles que tiene un usuarios
		for(GrantedAuthority rol : auth.getAuthorities()) {
			System.out.println("ROL: " + rol.getAuthority());
		}

		if(session.getAttribute("usuario") == null) {
			// con esta forma se van a almacenar los datos del usuario
	...
	// metodo que encripta las contraseñas de los usuarios
	@GetMapping("/bcrypt/{texto}")
	// para redendizar un texto y no una vista agregamos la notacion
	@ResponseBody
	public String encriptar(@PathVariable("texto")String texto) {
		return texto + " Encriptado en Bcrypt: " + passwordEncoder.encode(texto);
	}
	
	// metodo para mostrar el NUEVO LOGIN
	@GetMapping("/login" )
